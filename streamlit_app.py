...
# แต่งหน้าตาตัวหนังสือและธีม Dashboard ให้ดูสวยหรู ด้วย CSS
st.markdown("""
    <style>
...
# สร้างระบบสมุดบันทึก (Session State) ป้องกันข้อมูลหายเวลาขยับหน้าจอ
...
def predict_solar_irradiance(scaler_obj):
    ...
    if scaler_obj is not None:
        try:
            scaled = scaler_obj.transform(base_values.reshape(-1, 1)).flatten()
            real_pred = scaler_obj.inverse_transform(scaled.reshape(-1, 1)).flatten()
            return real_pred
        except:
            return base_values
    return base_values
